...
class Dataset_ASVspoof2019_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        X, _ = sf.read(str(self.base_dir / f"flac/{key}.flac"))
        X_pad = pad_random(X, self.cut)
        # x = self.smile.process_file(str(self.base_dir / f"flac/{key}.flac"))
        # x = np.asarray(x).reshape(88)
        x_inp = Tensor(X_pad)
        y = self.labels[key]
        return x_inp, y
...
class Dataset_train_prevDbs(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # x = self.smile.process_file(str(self.base_dir / f"{key}.wav"))
        X_pad = pad_random(X, self.cut)
        # x = np.asarray(x).reshape(88)
        x_inp = Tensor(X_pad)
        y = self.labels[key]
...
class Dataset_train_cs2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # x = self.smile.process_file(str(self.base_dir/key))
        # x = np.asarray(x).reshape(88)
        x_inp = Tensor(X_pad)
        y = self.labels[key]
        return x_inp, y
